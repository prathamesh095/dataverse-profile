...
/* --------------------------------------------------------------
   AnimatedButton – FIXED: NO w-full
   -------------------------------------------------------------- */
...
/* --------------------------------------------------------------
   ProjectActions – HORIZONTAL, CENTERED, RESPONSIVE
   -------------------------------------------------------------- */
    ...
    <div className="flex flex-wrap gap-3 items-center">
      ...
      <AnimatedButton size="sm" asChild>
        <Link href={`/projects/${project.id}`}>
          ...
          {isPreview ? "Full Details" : "Details"}
        </Link>
      </AnimatedButton>
      <AnimatedButton
        variant="outline"
  ...
  ({
    ...
    <div className="flex flex-col lg:flex-row gap-4 items-end justify-between">
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 flex-1 w-full">
        ...
        <div
          className="lg:col-span-1 flex gap-2"
          role="group"
          aria-label="Sort by"
        >
...
/* --------------------------------------------------------------
   ProjectPreviewContent – HORIZONTAL CENTERED BUTTONS
   -------------------------------------------------------------- */
const ProjectPreviewContent = memo(
      ...
      <>
        ...
        <div className="space-y-6 overflow-y-auto max-h-[70vh] pr-2">
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-2">
              {project.technologies.map((t) => (
                <Badge key={t} variant="outline" className="bg-primary/5">
                  {t}
                </Badge>
              ))}
            </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {Object.entries(project.metrics).map(([k, v]) => (
                <div
                  key={k}
                  className="text-center p-3 bg-muted/50 rounded-lg flex flex-col items-center"
                >
              ...
              <div className="space-y-3">
                {project.process.map((s, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="flex gap-3 items-start p-3 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors"
                    >
  ...
  (prev, next) =>
    prev.project.id === next.project.id && prev.onClose === next.onClose
);
ProjectPreviewContent.displayName = "ProjectPreviewContent";

/* --------------------------------------------------------------
   ProjectPreviewDialog – CLICK OUTSIDE TO CLOSE
   -------------------------------------------------------------- */
...
export function ProjectsGrid() {
  ...
  const filteredProjects = useMemo(() => {
    ...
    if (selectedDomain !== "all")
      list = list.filter((p) => p.domain === selectedDomain);
    if (debouncedSearch) {
  ...
  const stats = useMemo(() => {
    const avgYear =
      filteredProjects.length > 0
        ? Math.round(
            filteredProjects.reduce((s, p) => s + p.year, 0) /
      ...
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        >
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Explore real-world solutions across domains with live demos, code,
          ...
          <header className="flex justify-between items-center flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Filter and explore the full portfolio
                </p>
              </div>
            </div>
          </header>
